...
- source tracking values from the shared GitHub URL, falling back to the GitHub README path

If you arrived here from a community/share link, use the tracking values embedded in that GitHub URL. The URL query should include tracked_source, tracked_medium, tracked_campaign, and tracked_link_id before any # anchor. Do not rely on post text for source codes. If those values are missing, use the README defaults.
...
REPO_URL="${REPO_URL:-https://github.com/agent-valve-ai/Agent-Valve?tracked_source=github_readme&tracked_medium=github&tracked_campaign=agent_valve_survey_v1&tracked_link_id=github_readme_direct#direct-jotform-curl-submission}"
